	...
	<div class="main-content">
		...
		<div id="page-wrapper"  style="background:url('/admin/images/litiqs.jpg') no-repeat 100% 100%;">
			...
			<div class="form-body" data-example-id="simple-form-inline">
			  <form class="form-inline" action="/admin/admin_user">
			...
			<!-- 搜索 结束 -->
				<table class="table table-bordered" >
					...
					@foreach($admin_data as $v) 
						<tr class="th">
							<td>{{ $v->id }}</td>
							<td>{{ $v->name }}</td>
							<td><img src="/uploads/{{ $v->profile }}" width="60px" class="img-thumbnail" alt=""></td>
							<td>
							@if($v->status == 0)
					            超级管理员
					            @else($v->status == 1)
				...
				<div>{{ $admin_data->appends($params)->links()  }}</div>
